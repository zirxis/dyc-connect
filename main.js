...
function selectRole(role) {
    ...
    // Check if mobile
    if (isMobileDevice()) {
        // Show role selection modal on mobile
        showRoleModal(role);
    } else {
        // Show dashboard directly on desktop
        hideRoleSection();
        showDashboard(role);
    }
}
...
window.selectRoleAndContinue = selectRoleAndContinue;
window.continueWithRole = continueWithRole;
window.hideRoleSection = hideRoleSection;
...
function showRoleModal(role) {
    ...
    if (modal) {
        selectedRoleForModal = role;
        modal.classList.add('active');
        continueBtn.style.display = 'block';
        
        // Highlight selected role
        document.querySelectorAll('.role-selection-item').forEach((item, index) => {
            const roles = ['mother', 'teacher', 'specialist', 'institution', 'researcher'];
            if (roles[index] === role) {
                item.style.borderColor = '#00AEEF';
...
function selectRoleAndContinue(role) {
    selectedRoleForModal = role;
    const continueBtn = document.getElementById('continue-btn');
...
function continueWithRole() {
    if (selectedRoleForModal) {
        ...
        // Update modal title
        document.getElementById('role-content-title').textContent = roleTitles[selectedRoleForModal] || 'لوحة التحكم';
        
        // Update modal content
        document.getElementById('role-content-body').innerHTML = roleContent;
        
        // Show content step, hide selection step
        document.getElementById('role-selection-step').style.display = 'none';
        document.getElementById('role-content-step').style.display = 'block';
        
        // Animate content appearance
        anime({
            targets: '#role-content-step',
            opacity: [0, 1],
            duration: 300,
            easing: 'easeOutQuad'
        });
